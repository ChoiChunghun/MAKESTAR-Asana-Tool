...
export function TokenInput({ token, onTokenChange }: Props) {
  ...
  return (
    <div className="relative">
      {/* 배지 버튼 — 항상 표시, 헤더 높이 유지 */}
      <button
        type="button"
        onClick={() => { setDraft(token); setOpen((p) => !p); }}
      ...
      {/* 드롭다운 패널 — open 시 absolute 오버레이로 표시 */}
      {open && (
        <div className="absolute right-0 top-full mt-2 z-50 w-80 card shadow-lg border border-ms-border">
