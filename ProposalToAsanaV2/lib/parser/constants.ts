...
export const EXCLUDE_KEYWORDS_PC = [
  ...
  "이벤트",
  "키링",
  "사인",       // 사인 포토카드 = 서명 포토카드 → 디자인 작업 불필요 → 포토카드 분류 제외
...
export const BENEFIT_KEYWORDS = [
  "키링",
  // "사진" 단독 제외: 요청 포즈 사진 등 현장 촬영 사진은 디자인 작업 불필요 → 특전 아님
  // "인화" 붙은 형태만 특전으로 인정
  "인화 사진",
  "인화사진",
  "컷",
  ...
  "티켓",
  "L홀더",
  "카드 스티커",   // 스티커 단독 제외: 특정 스티커만 특전으로 인정
  "리폼 스티커",
  "데코 스티커",
  "판 스티커",
  "코스터",
  ...
  "우표",
  "뱃지",
  "USB 카드",      // 카드 단독 제외: USB 카드만 특전으로 인정 (메시지 카드는 별도 항목 유지)
  "usb 카드",
  "탑로더",
  "포스터",
  // "폴라로이드" 단독 제외: 사인·낙서 등 현장 서명 폴라로이드는 디자인 불필요 → 특전 아님
  // "인화 폴라로이드"만 특전 (인쇄/제작 필요)
  // "폴라로이드 포토카드"는 parsePhotocards에서 포토카드로 처리
  "인화 폴라로이드",
  "인화폴라로이드",
  "슬로건",
  "인화 2컷",
...
// 특전 라인 단순 제외 키워드 (이 단어가 포함되면 특전으로 파싱하지 않음)
export const EXCLUDE_KEYWORDS_BENEFIT = [
  "보드게임"   // 보드게임 관련 카드·소품은 특전 아님 (게임 진행용 도구)
];
...
export const EXCLUDE_PATTERNS_BENEFIT = [
  ...
  // 사인 폴라로이드 패턴 제거: BENEFIT_KEYWORDS에서 "인화 폴라로이드"만 유지하므로 불필요
];
...
export const EXCLUDE_PATTERNS_PC = [
  ...
  /^\s*>/                            // "> ~" 로 시작하는 이벤트 진행 설명문 (낙서 증정 조건 등)
];
