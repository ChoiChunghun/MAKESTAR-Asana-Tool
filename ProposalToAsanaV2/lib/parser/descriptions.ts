...
export function buildSnsOpenDescription(ctx: OpenContext): string {
  ...
  return `<body>${[
    ...
    "<strong>자료</strong><ol><li>URL 또는 MAS 위치</li></ol>",
    "<strong>문의</strong><ol>" +
      "<li>업로드 채널 (선택) : Instagram / X / Xiaohongshu / Weibo / VK</li>" +
      "<li>포카, 특전 블러 노출 여부 : X</li>" +
      "</ol>",
    "<strong>공유, 요청</strong><ol><li>소속사 요청 사항, 디자인 컨셉, 아이디어 등 공유가 필요한 내용을 편하게 남겨주세요!</li></ol>",
...
/**
 * 시트 언어 검수 태스크 설명 (메이크스타 오픈 전용)
 */
export function buildSiteLangDescription(): string {
  return (
    `<body><ul><li>사이트 URL:</li></ul>` +
    `\n<em>*담당자는 신예선/미주유럽사업팀으로 1차 기입해주시면 순차적으로 실 담당자가 배정됩니다.</em>` +
    `\n<em>*기한 지정은 PM이 진행합니다.</em>` +
    `\n<em>*언어 검수 완료 후 PM이 SNS 디자인 작업 시작을 요청합니다.</em>` +
    `</body>`
  );
}

/**
 * 메이크스타 어드민 상품 등록 태스크 설명
 */
export function buildAdminRegDescription(): string {
  return (
    `<body>` +
    `<em>*상품 등록을 위한 필수정보이니 빠짐없이 기입해주세요!</em>` +
    `\n<em>*담당자는 신예선/미주유럽사업팀으로 1차 기입해주시면 순차적으로 실 담당자가 배정됩니다.</em>` +
    `\n\n<strong>[필수] 앨범 정보</strong><ul>` +
    `<li>사이트 URL:</li>` +
    `<li>앨범 버전: 포토북, 포카앨범, 디지팩 등 오픈되는 앨범의 버전</li>` +
    `<li>앨범 버전명: 오픈되는 상세 버전명 (하위품목 SKU 매칭 목적)</li>` +
    `<li>판매 옵션: 판매 옵션 생성을 위한 전체 옵션수 &amp; 옵션별 상세 내용` +
    `<ul><li>옵션1:</li><li>옵션2:</li><li>...</li></ul></li>` +
    `</ul>` +
    `\n\n<strong>[필수] 사전 체크리스트</strong><ul>` +
    `<li>초도특전 여부:</li>` +
    `<li>활동 중단/탈퇴 멤버 여부:</li>` +
    `<li>통이미지 여부:</li>` +
    `<li>진행 언어:</li>` +
    `</ul>` +
    `\n\n<strong>[필수] 상품 가격</strong><ul>` +
    `<li>[국내, 해외 모두 기입] KRW:</li>` +
    `<li>[해외일 경우 기입]<ul><li>USD:</li><li>CNY:</li><li>JPY:</li></ul></li>` +
    `</ul>` +
    `\n\n<strong>[선택] 공유/요청사항</strong><ol>` +
...
/**
 * 웨이디엔(YDN) 어드민 상품 등록 및 검수 태스크 설명
 */
export function buildYdnAdminRegDescription(): string {
  return (
    `<body>` +
    `<em>*상품 등록을 위한 필수정보이니 빠짐없이 기입해주세요!</em>` +
    `\n<em>*담당자는 신예선/미주유럽사업팀으로 1차 기입해주시면 순차적으로 실 담당자가 배정됩니다.</em>` +
    ...
    `<li>사이트 URL:</li>` +
    `<li>앨범 버전: 포토북, 포카앨범, 디지팩 등 오픈되는 앨범의 버전</li>` +
    `<li>앨범 버전명: 하위품목 SKU 매칭을 위해 오픈되는 상세 버전명을 적어주세요` +
    ` (예: 6종 (멤버별 버전), 2종 (DAY-0 ver. / Night-Max ver.))</li>` +
    `<li>판매 옵션: 판매 옵션 생성을 위한 전체 옵션수 &amp; 옵션별 상세 내용` +
    `<ul><li>옵션1:</li><li>옵션2:</li><li>...</li></ul></li>` +
    `<li>오픈 언어: 한국어, 영어, 중국어(간체), 일본어 (해당하는 언어만 남겨주세요)</li>` +
    `</ul>` +
    `\n\n<strong>[해외일 경우] 국가별 가격</strong><ul>` +
    `<li>KRW:</li><li>USD:</li><li>CNY:</li><li>JPY:</li>` +
    `</ul>` +
    `\n\n<strong>[선택] 공유/요청사항</strong><ol>` +
    `<li>기타 등록 시 참고할 특이사항을 남겨주세요!</li>` +
    `</ol>` +
    `</body>`
  );
}
