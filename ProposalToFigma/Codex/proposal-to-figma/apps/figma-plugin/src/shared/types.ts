...
export type FigmaBinding = {
  ...
  sourceRange: ProposalRange;
  sourceHash: string;
  outputType: DesignOutput["outputType"];
  outputId: string;
  productCode: string;
  sourceRefs: string[];
  textBindings: Record<string, string>;
  ...
  createdAt: string;
  lastSyncedAt: string;
  manualOverrideLayers: string[];
};

export type SelectionBindingSummary = {
...
export type UiToPluginMessage =
  | { type: "INIT" }
  | { type: "SAVE_BACKEND_URL"; backendBaseUrl: string }
  | { type: "PARSE_SHEET"; backendBaseUrl: string; input: ParseSheetRequest }
  | { type: "GENERATE_OUTPUTS"; designData: DesignData; outputIds: string[] }
  | { type: "CHECK_UPDATES"; backendBaseUrl: string }
  | { type: "APPLY_UPDATES"; designData: DesignData; output: DesignOutput };

export type PluginToUiMessage =
  | { type: "BOOTSTRAP"; backendBaseUrl: string; selection: SelectionBindingSummary }
  | { type: "PARSE_RESULT"; payload: ParseSheetResponse }
  | { type: "GENERATE_RESULT"; created: Array<{ outputId: string; nodeId: string; nodeName: string }> }
  | { type: "SYNC_RESULT"; payload: DiffDesignResponse }
  | { type: "APPLY_RESULT"; nodeName: string; outputId: string }
  | { type: "SELECTION_CHANGED"; selection: SelectionBindingSummary }
